...
describe.only('Appointment Models: ', () => {
  describe('findFreeSchedule', () => {
    it('converts Gcal busytimes to Freetime during LG business hours', () => {
      ...
      // const currentTime = moment().startOf('day').tz('America/Los_Angeles').add({h:9.5})
      ...
      if( currentTime >= busyTimeData[0].start) {
  ...
  describe('findNextAppointment', () => {
    it('should find the first 30min appointment available', () => {
      ...
      const timeSlot = findNextAppointment(freeTime, currentTime, coachTestData)
      console.log('timeSlotTests', timeSlot)
    ...
    it('should find the first 30min appointment available in nth slot', () => {
      const result = findNextAppointment(freeTimeData2, moment(), coachTestData)
